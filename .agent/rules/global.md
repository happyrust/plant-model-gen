...
> **代码检索规则**：进行代码库检索时，必须优先使用 `ace-tool` 做首轮语义检索与定位；`grep`/`rg` 只能作为 `ace-tool` 之后的二次确认工具，除非已经明确知道目标标识符，或任务明确要求穷举精确文本匹配。
> **测试验证规则**：禁止创建或运行基于 `cargo test` 的测试用例；本项目验证必须通过 CLI 命令或启动服务后经 HTTP 访问/接口调用完成。
