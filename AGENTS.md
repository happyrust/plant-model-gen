...
## 构建、测试与开发命令
`cargo build --all-features` 编译完整功能集，推荐在提交前执行以验证依赖特性互相兼容。`cargo run --bin web_ui --features web_ui` 启动内置 Web UI 调试界面，默认读取 `DbOption.toml` 中的配置。核心回归测试使用 `cargo test --all-targets`，并在需要重放模型导出流程时运行 `./run_model_gen_test.sh`。若需复现 XKT 生成链路，可执行 `./test_xkt_generation.sh` 或 `node test_complete_flow.js`。
...
## 配置与环境提示
默认配置文件位于仓库根目录的 `DbOption*.toml`，本地调试请复制 `DbOption.toml` 为私有版本并避免提交敏感路径。运行需要外部依赖（如 SurrealDB、MQTT 或 LiteFS）时，可使用 `start_surreal_with_check.sh` 与 `litefs-start.sh` 快速拉起必要服务；在 CI 环境请禁用 `--features web_ui` 以缩短构建时间。
