...
- `api_request_log.rs`(新):review/platform 域最外层中间件采集 request/response(4KB 截断+truncated 标记、状态码、耗时、request_id,query/path/body 提取 form_id/task_id);丢弃请求头,token/password/secret 字段打码;异步写 Surreal `api_request_log`(迁移 `20260610_002`),失败仅告警;保留 7 天惰性清理。
- `logs_api.rs`(新):`GET /api/logs/types`(按 JWT Role 裁剪)+ `GET /api/logs?type=...`;adapter 适配 `api.request` / `review.workflow`(复用 review_workflow_history)/ `server.runtime` / `site.file.{parse,generate,db,web,viewer}`(复用站点日志 tail);统一 LogEntry 与游标分页;接口/站点/进程日志仅管理角色可见。
- `logs_api.rs`(spec 004):`site.file.*` 未传 site_id 默认 `current_site_id()`;隐式兜底命中非 managed 站点降级空列表,显式传参语义不变。
- `server_runtime_log.rs`(新,spec 005):TeeLogger 包装 env_logger,info+ 写入 5000 条环形缓冲(单条 2KB),新类型 `server.runtime` 支持 level/q/时间窗/游标;控制台输出行为不变。
- 验收:隔离环境 HTTP 冒烟 + 真实站点回归 + Playwright 浏览器实测全过,详见 `specs/003-review-log-viewer` / `004-log-site-visibility` / `005-server-runtime-logs`。

### Changed — SurrealDB 客户端升级 3.1.0-alpha → 3.2.0-nightly

> 全量 `cargo update`;协议与数据格式影响已实测并形成切换 Runbook。

- surrealdb 系列指向 fork dev-3.1 `#f01470af`(内部成员 3.2.0-nightly),RocksDB 10.6→11.0,revision 0.17→0.28,共 200+ 包刷新。
- ⚠️ 与 3.1.0-alpha server WS 协议不兼容(`Failed to decode value from fb value`),客户端/服务端必须同步升级;旧 rocksdb 数据可被 3.2 直接打开(956 万条 pe 副本实测)但属单向格式升级,切换前必须冷备份。
- Windows debug 版 surreal server 需 `/STACK:16777216` 重链接;生产使用 release 构建。
- 切换步骤见 `docs/plans/2026-06-11-surrealdb-32-upgrade-runbook.md`。

## 2026-06-10

### Changed — review 域 SurrealDB 上下文韧性(spec 002)

- `review_db.rs`:review 主库改为进程级 OnceCell + 按调用克隆会话,移除每请求 `USE NS/DB`;`review.ensure_context` 不再作为请求级超时点;WebSocket 模式进程级一次性心跳保活。
- review schema 迁移链落地:`review_schema_migrations` 表 + `rs_surreal/review/migrations/`。

### Added — CATA 按需闭包 manifest 生成 CLI 入口(T006c,feat/on-demand-cata-closure 分支)

- 新增子命令 `gen-cata-closure`(`--rescan-index` / `--out`):db_index.sqlite 缺失自动全量预扫 → 扫描工程根全部 DESI → refno 级闭包 → 原子写 `output/<project>/scene_tree/cata_closure.json`;配合 `AIOS_CATA_CLOSURE_MODE=manifest` 实现 CATA 部分解析。
...
> 修复跨仓库 surrealdb 依赖来源不一致导致的 `T: SurrealValue` trait bound 不满足问题：rs-core 走 `https://github.com/happyrust/surrealdb`，而本仓 / `pdms-io-fork` 走 `https://gitee.com/happydpc/surrealdb`，cargo 解析后会拉两份独立的 `surrealdb_types` crate 实例，`SurrealValue` 派生与下游消费方位于不同 trait 路径下，触发 147 个 E0277。
