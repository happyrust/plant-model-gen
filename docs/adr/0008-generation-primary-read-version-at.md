...
> 2026-07-23 修订：多库轮次不再分别取“本库提交时刻”，而是统一取整轮最后一个数据锚点的数据库时间；正式站点的 `allow-full-regen` 语义收敛到绑定既有数据锚点且有运行审计的受控 catch-up/repair，任意 generate/regen 被禁止。详见 ADR-0010。

被否决的替代方案：① 纯活读不钉时刻——省一个时间戳换一个静默坑；② 保留副本表改由增量直接喂——白养五张表与复制链，只为躲开本来就在生产跑的主表读。

多库增量失败时整轮禁生成、成功轮次统一读取时刻、no-op 模型锚点及正式站点受控 repair/catch-up 边界见 ADR-0010。
