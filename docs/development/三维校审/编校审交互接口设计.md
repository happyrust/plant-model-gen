...
- `project_id`: 项目号
- `user_id`: 用户ID
- `user_name`: 用户姓名（可选；未传时回退为 `user_id`）
- `form_id`: 表单ID
- `role`: 角色（可选）
- `workflow_mode`: 工作流模式（可选，`external | manual | internal`）
...
`form_id / project_id / user_id / output_project` 不再作为正式路由参数真值；如需保留，仅存在于接口响应体调试字段中。
...
{  
    ...
    "token": "eyJhbGciOi...", //平台生成并签名的 JWT；服务端会做签名校验并校验 form_id 一致性  
...
token：平台按照上述 JWT 规则生成的 token 值（`Header.Payload.Signature`），模型服务会校验签名有效性；
...
当前实现说明：
- `UCode` 与 `UKey` 都会做值校验，而不只是检查 Header 是否存在。
- 平台侧默认从环境变量 `AUX_DATA_UCODE`、`AUX_DATA_UKEY` 读取期望值；未配置时 `AUX_DATA_UCODE` 默认回退为 `ZY`，`AUX_DATA_UKEY` 默认回退为空字符串。
...
```json
{
    ...
    "token": "token", //按照"Token 认证方案 (JWT)"章节所述规则生成并签名（需与 form_id 一致）
...
当前实现说明：
- `token` 使用 JWT 校验签名与过期时间；并额外校验 token claims 中 `form_id` 与请求体 `form_id` 一致。
- 现阶段 `sh` 与 `pz` 节点共用同一负责人字段 `approver_id`；如果后续业务要求“审核”和“批准”拆分为不同人员，再单独引入新的负责人字段。
- `comments` 表示平台流程引擎的当前节点整体审批意见，模型中心只把它当作 workflow 动作上下文输入；不再在模型中心持久化，也不会在 `workflow/sync` 响应中回传。

模型中心处理结束后，需要将当前单据所对应的**模型侧校审数据包**进行返回。返回内容聚焦模型文本批注、云线、框选/OBB、测量、批注评论串、文档/图片附件等模型数据；平台流程意见不再由该接口回传。
...
```json
{
  ...
  "message": "ok",
  "data": {
    "models": ["ref-001", "ref-002", "ref-003"],
    "task_id": "task-123",
    "records": [
      {
        "id": "record-001",
        ...
        "measurements": [],
        "note": "模型侧复核备注",
        "confirmed_at": "2026-03-27 11:45:19"
      }
    ],
    "annotation_comments": [
      {
        ...
        "author_name": "康某",
        "author_role": "sj",
        "content": "请结合土建条件再复核一次",
        "reply_to_id": null,
        "created_at": "2026-03-27 11:45:20"
      }
    ],
    "attachments": [
      {
        ...
        "id": "file-001",
        "type": "markup",
        "route_url": "/files/review_attachments/file-001.png",
        "download_url": "/files/review_attachments/file-001.png",
        "public_url": "http://host/files/review_attachments/file-001.png",
        "description": "云线截图",
        "file_ext": ".png"
      }
    ]
...
{  
    ...
    "token": "token" //按照"Token 认证方案 (JWT)"章节所述规则生成并签名
}
```
