#!/usr/bin/env bash
# Ubuntu 生产环境：固定用 RocksDB 数据目录，监听 0.0.0.0:8020（与 DbOption surreal_port / 前端 VITE_SURREAL_URL 一致）。
...
# 与历史部署 surrealdb.service 中数据路径保持一致
SURREAL_STORAGE="${SURREAL_STORAGE:-rocksdb:///var/lib/surrealdb/data}"
