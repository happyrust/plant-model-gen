...
**状态**: 骨架（2026-07-16）。M1+M2 落定、M3 CLI 可用后，按本手册逐步补齐命令实测输出与附录（对应 tasks.md T021/T023）。
...
- **retention 默认 90d**：`retention=0` 为无限保留（磁盘风险，需显式确认）。改 retention 只影响 GC 水位线推进，不需要重建库。
...
- 参考脚本：`db-data/run_surrealkv_versioned.ps1`（起 versioned 实例）、`db-data/test_version_data.surql`（VERSION 查询基线）、`db-data/verify_versioned_pe_att.surql`（T019 扩展脚本，M3 阶段产出）。

## Scenario 1: 新建 versioned 数据目录 + 站点配置开关

1. 为站点分配一个**全新**数据目录（不复用旧目录）。
2. 站点 `DbOption.toml`（或管理端站点编辑接口，见 T022）设置：
...
> ⚠️ T022（站点编辑防护）落定前，已初始化站点直接改 `versioned_storage` 不会自动重灌；本手册要求手动新建目录。T022 落定后，管理端对已初始化站点改此开关会返回「需要重灌数据目录」提示，不静默改参数。
...
**Expected**：抽样 refno 集合当前态逐字段一致（SC-004）。差异清单记入本文附录。

## Scenario 4: 切流 + 增量回归

1. 将站点流量切到新 versioned 实例。
...
- 模型生成、导出全链路无回归。
- （M3 CLI 可用后）`model-version history snapshot --refno <E> --sesno <N>` 能返回改前快照。
