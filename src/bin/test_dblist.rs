...
fn print_elements_tree(elements: &[aios_core::dblist_parser::PdmsElement], depth: usize) {
    ...
    for element in elements {
        ...
        for (i, (name, value)) in element.attributes.map.iter().enumerate() {
            if i >= 5 {
                println!("{}  ... 还有 {} 个属性", indent, element.attributes.map.len() - 5);
                break;
            }
...
/// 递归保存所有元素到数据库
async fn save_all_elements_recursive(elements: &[aios_core::dblist_parser::PdmsElement]) -> Result<i32> {
    save_elements_recursive_helper(elements).await
}

/// 辅助函数处理递归保存
async fn save_elements_recursive_helper(elements: &[aios_core::dblist_parser::PdmsElement]) -> Result<i32> {
    ...
    for element in elements {
        ...
        // 打印转换后的数据结构
        println!("\n📋 保存元素: {} ({})", element.get_noun(), element.get_refno_string());
        ...
        // 递归保存子元素
        if !element.children.is_empty() {
            let child_count = Box::pin(save_elements_recursive_helper(&element.children)).await?;
...
async fn verify_loaded_data() -> Result<()> {
    // 查询总数 - 使用聚合查询
    let total_count: Option<i64> = SUL_DB.query_take("SELECT VALUE count() FROM ONLY (SELECT count() FROM pe GROUP ALL)", 0).await?;
    println!("📊 数据库验证:");
...
/// 将 PdmsElement 转换为数据库格式
fn convert_element_to_db_format(element: &aios_core::dblist_parser::PdmsElement) -> serde_json::Value {
    let mut data = json!({
    ...
    if let Some(attributes_obj) = data.as_object_mut() {
        ...
        for (name, value) in &element.attributes.map {
            attrs_map.insert(
                name.clone(),
                convert_named_attr_value_to_json(value),
            );
        }
...
fn convert_named_attr_value_to_json(value: &aios_core::NamedAttrValue) -> serde_json::Value {
    ...
    match value {
        ...
        NamedAttrValue::F32Type(f) => serde_json::Value::Number(serde_json::Number::from_f64(*f as f64).unwrap_or(serde_json::Number::from(0))),
        ...
        NamedAttrValue::Vec3Type(v) => serde_json::Value::Array(vec![
            serde_json::Value::Number(serde_json::Number::from_f64(v.x as f64).unwrap_or(serde_json::Number::from(0))),
            serde_json::Value::Number(serde_json::Number::from_f64(v.y as f64).unwrap_or(serde_json::Number::from(0))),
            serde_json::Value::Number(serde_json::Number::from_f64(v.z as f64).unwrap_or(serde_json::Number::from(0))),
        ]),
        NamedAttrValue::IntArrayType(arr) => serde_json::Value::Array(
            arr.iter().map(|&i| serde_json::Value::Number(serde_json::Number::from(i))).collect()
        ),
        NamedAttrValue::StringArrayType(arr) => serde_json::Value::Array(
            arr.iter().map(|s| serde_json::Value::String(s.clone())).collect()
        ),
        NamedAttrValue::BoolArrayType(arr) => serde_json::Value::Array(
