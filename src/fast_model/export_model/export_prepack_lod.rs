...
pub async fn export_prepack_lod_for_refnos(
    ...
    let equi_children = {
        ...
        for chunk in expanded_refnos.chunks(SQL_BATCH_SIZE) {
            let owner_refnos = chunk.iter().map(|r| r.to_pe_key()).collect::<Vec<_>>();
            let sql = format!(
                r#"
    ...
    let (geo_hashes, geo_index_map) = if !generated_assets.is_empty() {
        ...
        let index_map: HashMap<String, usize> = hashes
            .iter()
            .enumerate()
            .map(|(i, h)| (h.clone(), i))
            .collect();
                ...
                for geom in &component.geometries {
                    if let Some(&geo_index) = geo_index_map.get(&geom.geo_hash) {
                        ...
                        if verbose && !geom.geo_hash.contains('_') {
                            ...
                            println!(
                                "   🔍 [BRAN] geo_hash={} unit_flag={} effective={}",
                                geom.geo_hash, geom.unit_flag, eff_flag
                            );
                        }
...
async fn collect_deep_visible_refnos(
    ...
    if let Some(filter) = filter_nouns {
        let filter_set: HashSet<String> = filter.iter().map(|t| t.to_uppercase()).collect();
...
fn aabb_to_json(
    aabb: &aios_core::types::PlantAabb,
    unit_converter: &UnitConverter,
) -> serde_json::Value {
...
pub async fn export_dbnum_instances_json(
    ...
    for owner_refno in &owner_refnos {
        ...
        for child in &owner_group.children {
            ...
            let first_geo_hash = export_inst
                .insts
                .first()
                .map(|i| i.geo_hash.clone())
                .unwrap_or_default();

            let compact_geo_instances: Vec<CompactGeoInstance> = export_inst
                .insts
                .iter()
    ...
    let exported_children: usize = groups
        .iter()
    ...
    let exported_tubings: usize = groups
        .iter()
...
pub async fn export_dbnum_instances_json_from_cache(
    ...
    let mut ngmr_neg_relate_map: HashMap<RefnoEnum, Vec<(RefnoEnum, RefnoEnum)>> = HashMap::new();
    ...
    if verbose {
        ...
        println!("✅ 缓存 refno 数量: {}", cached_refnos.len());

        let total_inst_geos: usize = inst_geos_map.values().map(|v| v.insts.len()).sum();

        let empty_inst_geos = inst_geos_map
            .values()
            .filter(|v| v.insts.is_empty())
            .count();
    ...
    fn resolve_aabb(
        ...
        fn compute_inst_aabb(
            info: &EleGeosInfo,
            inst: &aios_core::geometry::EleInstGeo,
        ) -> Option<Aabb> {
            ...
            if points.is_empty() {
                ...
                match &inst.geo_param {
                    PdmsGeoParam::PrimExtrusion(e) => {
                        ...
                        let min_z = z_candidates.iter().cloned().fold(f32::INFINITY, f32::min);
    ...
    for owner_refno in &owner_refnos {
        ...
        let mut ordered_child_refnos: Vec<RefnoEnum> =
            match collect_export_refnos(&[*owner_refno], true, None, false).await {
        ...
        for child_refno in &ordered_child_refnos {
            ...
            let compact_geo_instances: Vec<CompactGeoInstance> = inst_geos
                .insts
                .iter()
                .map(|inst| {
                    let geo_hash = if inst.geo_hash < 10000 {
...
pub async fn export_global_trans_aabb_json(
    ...
    let mut trans_table: std::collections::HashMap<String, serde_json::Value> =
        std::collections::HashMap::new();
    ...
    let mut aabb_table: std::collections::HashMap<String, serde_json::Value> =
        std::collections::HashMap::new();
...
fn write_prepack_parquet_and_patch_manifest(output_dir: &Path) -> Result<()> {
    ...
    let geom_manifest: PrepackGeometryManifest =
        serde_json::from_slice(&fs::read(&geometry_manifest_path)?)?;
    ...
    write_geometry_manifest_parquet(
        output_dir.join(geometry_manifest_parquet_name),
        &geom_manifest,
    )?;
...
#[cfg(feature = "parquet-export")]
fn write_geometry_manifest_parquet(
    path: PathBuf,
    manifest: &PrepackGeometryManifest,
) -> Result<()> {
    let mut geo_hashes: Vec<Option<String>> = Vec::new();
    ...
    let mut sphere_center_builder =
        FixedSizeListBuilder::new(PrimitiveBuilder::<Float32Type>::new(), 3)
            .with_field(Arc::new(Field::new_list_field(DataType::Float32, false)));
    ...
    for &refno in refnos {
        ...
        match TreeIndexManager::resolve_dbnum_for_refno(refno) {
            ...
            Err(e) => {
                if verbose {
                    eprintln!(
                        "[instances] 无法解析 refno 的 dbnum: refno={} err={}",
                        refno, e
                    );
                }
            }
