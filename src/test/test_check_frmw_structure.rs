...
mod tests {
    use aios_core::{RefnoEnum, SurrealQueryExt, init_surreal, project_primary_db};
    use anyhow::Result;
    use std::str::FromStr;
    ...
    async fn test_check_frmw_structure() -> Result<()> {
        ...
        let result1: Vec<serde_json::Value> = project_primary_db()
            .query_take(&sql1, 0)
            .await
            .unwrap_or_default();
        ...
        let result2: Vec<serde_json::Value> = project_primary_db()
            .query_take(&sql2, 0)
            .await
            .unwrap_or_default();
        ...
        let result3_1: Vec<RefnoEnum> = project_primary_db()
            .query_take(&sql3_1, 0)
            .await
            .unwrap_or_default();
        println!(
            "   3.1 PANE WHERE OWNER.OWNER = FRMW: {} 个",
            result3_1.len()
        );
        ...
        let result3_2: Vec<RefnoEnum> = project_primary_db()
            .query_take(&sql3_2, 0)
            .await
            .unwrap_or_default();
        ...
        let result3_3: Vec<serde_json::Value> = project_primary_db()
            .query_take(&sql3_3, 0)
            .await
            .unwrap_or_default();
        ...
        let result4: Vec<RefnoEnum> = project_primary_db()
            .query_take(&sql4, 0)
            .await
            .unwrap_or_default();
        ...
        let result5: Vec<serde_json::Value> = project_primary_db()
            .query_take(&sql5, 0)
            .await
            .unwrap_or_default();
        ...
        let result6: Vec<serde_json::Value> = project_primary_db()
            .query_take(&sql6, 0)
            .await
            .unwrap_or_default();
