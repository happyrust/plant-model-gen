...
async fn test_specific_room_rebuild_with_known_baseline() -> anyhow::Result<()> {
    ...
    // 注意：这些房间号需要真实存在于数据库中
    let test_room_numbers = vec!["R610".to_string(), "R661".to_string()];

    let stats =
        rebuild_room_relations_for_rooms(Some(test_room_numbers.clone()), &db_option).await?;
...
async fn test_room_worker_rebuild_all_task() -> anyhow::Result<()> {
    use aios_core::get_db_option;
    use aios_database::fast_model::{RoomTaskType, RoomWorker, RoomWorkerConfig, RoomWorkerTask};
